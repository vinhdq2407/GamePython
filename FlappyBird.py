...
IMG_BIRD_1 = pygame.image.load('FlappyBird/code/img/bird1.png') # Ảnh chim 1
IMG_BIRD_2 = pygame.image.load('FlappyBird/code/img/bird2.png') # Ảnh chim 2
IMG_COT = pygame.image.load('FlappyBird/code/img/column.png') # Ảnh cột
...
class Score(Text):
    def __init__(self, text,x = WINDOWWIDTH/2, y = WINDOWHEIGHT/6, size = 50, color = RED):
...
class ClickToST(Text): # Chữ nhấp nháy
    def __init__(self, text, y = WINDOWHEIGHT*0.6, size = 40, color = RED):
...
class Heading(Text): # Tiêu đề (chữ to)
    def __init__(self, text, y = -100, size = 90, color = RED):
        super().__init__(text, WINDOWWIDTH/2 - len(text)/2, y, size, color)
        self.speed = 0
...
def main():
    ...
    logo = pygame.image.load('FlappyBird/code/img/logo.png')
    pygame.display.set_icon(logo)
    IMG_BG = pygame.image.load('FlappyBird/code/img/chi1.jpg').convert()
    scene = Scenes()
    bird = Bird()
    columns = Columns()
    score = Score('0')
